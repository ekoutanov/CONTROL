...
## Abstraction
Before entering delivery, the nominated PO firms up an _abstract_ requirements specification, including nonfunctional concerns, which are ultimately signed off by the business.
...
With the benefit of the delivery squad model, CONTROL attacks the biggest source of unpredictability in Agile — estimation. When we say, for example, that a ticket is a five-pointer, we have no clue what that means in practice. Agile estimation assumes that some abstract developer will work on the ticket in question because we have no knowledge of who that developer will be. People cannot be abstracted this way, and there is no "average" developer. It's not even a matter of skill: people who are familiar with certain tasks will accomplish them quickly. In a delivery squad model, we can often tell exactly who that developer will be. Even in large delivery squads, only a few people will work on any given component. So, not only can we estimate with _a priori_ knowledge of these people, but it is them who will produce the estimates in the Formation phase. As a member of a delivery squad, you are almost always estimating on your behalf.
