...
Suppose Bookworm Inc. wants to display the supply date for books that are out of stock. It's a straightforward change on the web storefront, and the data is available in the back-end warehouse management system — but their API does not presently expose this. There are two small changes required: 1) on the storefront, to display the data, and 2) on the back-end API, to expose the data.

According to the CONTROL model, the change above is classed as a capability delivery, albeit a simple one. It will ideally follow the same process model as any other change. In saying this, we bring to the forefront one of the central takeaways of Chapter 5: _CONTROL phases can be as lightweight as the team allows them to be_. The Bookworm Inc. enhancement feels like a genuinely low-risk activity, so we treat it as such. The CONTROL phases will be largely expedited. The business will rubber-stamp the requirements during the Abstraction phase as they likely instigated the change to begin with. The Elaboration phase is led by a senior member of the storefront team, acting in the PE role<sub>2</sub>. Architecture will likely approve the API change proposed casually by the storefront team. Formation is as simple as assigning a ticket to one member from each team and ensuring that valuable context isn't lost. Construction is just completing the tickets at an opportune moment. The risks are so low that project tracking is purely by exception — in other words, should some incident occur (e.g., a blocker), the impacted developer will notify the accountable person in the storefront team — the quasi-PE — who will act accordingly to resolve the problem.
...
The convenience of CONTROL's risk-centric approach cannot be overstated. There is no limit to how lightweight phases may be. (Or heavyweight, for matter.) The point isn't to create vacuous commotion but provide a straightforward framework for orderly decision-making.
